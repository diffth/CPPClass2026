...
//      구조체(struct) : 여러 변수 타입들의 묶음. struct Info { int age, float height };
...
//      소프트웨어 설계할 때 공통적으로 발행하는 문제에 대한 재사용을 가능하게 하는 해결책
//      기능이나 알고리즘 해법 제시 x
//      특정 상황에서 구조적인 문제를 해결 o
//      언리얼에서는 싱글턴(singleton) 패턴을 주로 사용.
//                     
// 
//---------------------------------------------------------------------------
//6.자료구조(Data Structure)
//   선형구조
//      Vector  : 배열 + (가변길이, 동적할당), 인덱스로 검색
//      List    : 노드간 주소로 연결. 연결된 주소를 순회하며 검색
//      Stack   : 후입선출(LIFO), 중간요소 삽입/삭제 불가
//      Queue   : 선입선출(FIFO), 중간요소 삽입/삭제 불가
// 
//   비선형구조
//      Tree 구조. 부모노드/자식노드 관계 구조
//          Set : key만. key중복허용x. key로 검색
//          Map : key/value 쌍. key중복허용x. key로 검색
//      Graph구조
//          정점(vertex)  : 노드. 그래프 구성요소
//          간선(edge)    : 정점간 연결관계 요소
//          가중치(weight): 간선의 크기값 요소
// 
// 
//---------------------------------------------------------------------------
//7.STL 컨테이너(STL Container)
//   컨테이너(Container) : c++에서 데이터(객체)를 저장하고 관리하는 표준 자료구조 집합. 
// 
//   <vector>
//   <list>
//   <stack>
...
//   <map>, <pair>
//   
//   <vector> vector 자료구조를 담고 있는 컨테이너
//      index로 요소 접근
//      반복문을 통한 벡터 접근
//          반복자 기반 반복문  iterator반복자 사용
//          범위 기반 반복문    foreach문 사용
//   
//   <list> list 자료구조를 담고 있는 컨테이너
//      index로 요소 접근 불가. sort(); //정렬 remove(); //요소제거
//      반복문을 통한 list 접근
//          반복자 기반 반복문  iterator반복자 사용. reverse_iterator(역반향 반복자)
//          범위 기반 반복문    foreach문 사용
// 
//   <stack> stack 자료구조를 담고 있는 컨테이너
//      LIFO. 제한된 스택 선언, 초기화, 요소 접근
//      LIFO. 제한된 스택 삽입/삭제.
// 
//   <queue> queue 자료구조를 담고 있는 컨테이너
//      FIFO. 제한된 스택 선언, 초기화, 요소 접근
//      FIFO. 제한된 스택 삽입/삭제.
//   
//   <set> set 자료구조를 담고 있는 컨테이너
//      key만 요소로 갖고 있다.
//       
//   <pair>, <map> pair/map 자료구조를 담고 있는 컨테이너
//      key와 value를 요소로 갖고 있다.
//      pair는 <key, value> 로 이루어진 자료구조의 자료 하나.
//      map 은 <key. value> 를 갖는 자료구조. pair를 요소로 갖는다.
//      중복된 키가 있으면, 무시된다.(key 중복허용x)
// 
// 
//---------------------------------------------------------------------------
//기타등등
